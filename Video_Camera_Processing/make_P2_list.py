...
def rotation_look_forward_x():
    """Rotation so camera looks straight forward along x axis (view direction = x in world)."""
...
# First camera: look straight forward (along x axis). Second camera: look_at target.
TARGET_LOOK_AT = np.array([0.0, 0.0, 2.0])

cameras = [
    {
        "fx": 3400.0, "fy": 3400.0, "cx": 2048.0, "cy": 1500.0,
        "R": rotation_look_forward_x(),
        "t": [0.0, -7.5, 2.0],
    },
    {
        "fx": 4800.0, "fy": 4800.0, "cx": 1920.0, "cy": 1080.0,
        "R": look_at([23.8485, -7.5, 2.0], TARGET_LOOK_AT),
        "t": [23.8485, -7.5, 2.0],
    }
]
...
ax.legend()
ax.set_title("Camera positions and viewing direction (Cam0: look -x, Cam1: look_at)")
plt.tight_layout()
